...
export default function DepositIssuanceLeaderboard() {
  const { user } = useApp();
  ...
  return (
    <div className="space-y-4 px-5 pb-8 pt-2" data-testid="deposit-issuance-leaderboard-page">
      <section className="flex items-start gap-3">
        <button type="button" onClick={() => navigate("/goals/deposit")} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-[#1A1A1E] text-zinc-300 active:scale-95"><ArrowLeft size={21} strokeWidth={2.7} /></button>
        <div className="min-w-0 flex-1">
          <h1 className="font-display text-[26px] leading-tight text-white">Рейтинг депозитних видач</h1>
      ...
      {!rows.length ? (
        <section className="rounded-3xl border border-white/10 bg-[#1A1A1E] p-7 text-center">
          ...
          <h2 className="mt-3 font-display text-xl text-white">РЕЙТИНГ ВИДАЧ ЩЕ НЕ ЗНАЙДЕНО</h2>
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#1A1A1E]">
          <div className="flex items-center justify-between p-4"><div><h2 className="font-display text-xl text-white">Рейтинг операторів за видачами</h2><p className="mt-1 text-xs text-zinc-500">Сортування за загальним підсумком</p></div><UsersRound size={21} className="text-[#39FF14]" /></div>
