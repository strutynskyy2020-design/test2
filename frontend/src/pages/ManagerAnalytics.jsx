    ...
    <div className="space-y-4 px-5 pb-8 pt-2" data-testid="manager-analytics-page">
      ...
      {data && (
        <>
          ...
          <Panel title="Тренд Google-звітів" subtitle="Кредитний і дебетовий підсумок після натискання «Оновити звіти»" icon={TrendingUp}>
            {reportTrend.length ? <div className="h-60 w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={reportTrend} margin={{ left: -18, right: 8, top: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,140,.18)"/><XAxis dataKey="label" tick={{ fontSize: 8, fill: "#71717A" }}/><YAxis tick={{ fontSize: 9, fill: "#71717A" }}/><Tooltip contentStyle={{ borderRadius: 14, border: "1px solid #E4E4E7", fontSize: 11 }}/><Legend wrapperStyle={{ fontSize: 10 }}/><Area type="monotone" dataKey="credit_overall" name="Кредит" stroke="#8B5CF6" fill="#8B5CF622" strokeWidth={2.5}/><Area type="monotone" dataKey="debit_overall" name="Дебет" stroke="#0891B2" fill="#0891B222" strokeWidth={2.5}/></AreaChart></ResponsiveContainer></div> : <div className="py-6 text-center text-xs font-bold text-zinc-500">Тренд з’явиться після публікації наступних знімків Google-звітів.</div>}
          </Panel>
        </>
      )}
    </div>
