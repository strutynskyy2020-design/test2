...
assert(!code.includes('REPORT_SOURCE_SPREADSHEET_ID'), 'Legacy report-source split constant must stay removed');
assert(code.includes('ACTIVATORS_SPREADSHEET_ID'), 'V155 must define the dedicated activators workbook');
...
const activatorSheetNames = [
  'Pumb Online',
  'Pumb Online transformation',
  'Giving Pumb Online',
  'Card activation',
  'Card transformation',
  'Giving card',
...
const activators = fakeSpreadsheet(
  activatorSheetNames.map((name) => ({ name, hidden: false, display: [], raw: [] }))
);
...
const snapshot = sandbox.buildReportSnapshots(ss, activators);
...
console.log('V155 main-source + separate activators compatibility tests passed');
