...
function activationPeriodFromTitle(value) {
  const key = normalizeHeaderKey(value);
  // The source spreadsheet historically uses the misspelling "yestarday".
  // Keep the aliases explicit so a period title can never be treated as part
  // of the preceding monthly block.
  if (
    key.includes("yesterday") || key.includes("yestarday") || key.includes("yesturday") ||
    key.includes("вчора") || key.includes("вчера")
  ) return "yesterday";
  if (key.includes("month") || key.includes("monthly") || key.includes("міся")) return "month";
  return "";
}

function isActivationTransformBlockTitle(value, kind) {
  const key = normalizeHeaderKey(value);
  if (kind === "pumb") {
...
function isActivationTransformTitle(value, kind) {
  return Boolean(activationPeriodFromTitle(value)) && isActivationTransformBlockTitle(value, kind);
}
