...
const REPORT_CACHE_API_VERSION = "v105-team-aware-reports";
...
function teamReportKey(value) {
  const normalized = normalizeHeaderKey(value);
  const match = normalized.match(/(?:^|_)(?:tm|тм)_?(\d+)$/i);
  ...
  for (let rowIndex = blockRowIndex; rowIndex <= endRow; rowIndex += 1) {
    ...
    for (let columnIndex = 0; columnIndex < row.length; columnIndex += 1) {
      ...
      if (teamKey) {
        ...
        if (defaultColumn === -1 && teamKey === "tm6") defaultColumn = columnIndex;
      }
      if (defaultColumn === -1 && (key.includes("загальний") || key.includes("general") || key.includes("overall"))) {
        defaultColumn = columnIndex;
      }
    }
  }
  if (defaultColumn === -1) {
    const firstKey = Object.keys(teamColumns)[0];
    if (firstKey) defaultColumn = teamColumns[firstKey];
  }
  return { defaultColumn, teamColumns };
}
