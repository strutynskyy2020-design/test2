...
const sandbox = {
  ...
  Utilities: {
    formatDate: (_date, _timezone, format) => format === "yyyy-MM-dd" ? "2026-07-31" : "31.07.2026 16:35",
...
assert.equal(manifest().snapshot_day, "2026-07-31");
assert.equal(manifest().api_version, "v103-daily-preloaded");
...
const reportPages = noPollingPages.map((page) => fs.readFileSync(`frontend/src/pages/${page}`, "utf8"));
for (const [index, source] of reportPages.entries()) {
  assert(source.includes("useDailyGoogleReports"), `${noPollingPages[index]} does not use the shared daily report cache`);
  assert(!source.includes('/.netlify/functions/google-goals"'), `${noPollingPages[index]} still fetches reports directly`);
}
const cacheSource = fs.readFileSync("frontend/src/lib/googleReportsCache.js", "utf8");
assert(cacheSource.includes("snapshot_version"), "browser cache does not compare published snapshot versions");
assert(cacheSource.includes("localStorage"), "browser report cache is not persistent");
assert(cacheSource.includes("inFlightReports"), "browser report requests are not deduplicated");
const versionGatewaySource = fs.readFileSync("netlify/functions/google-goals-version.js", "utf8");
assert(versionGatewaySource.includes("public, durable"), "manifest endpoint does not use Netlify durable caching");
assert(versionGatewaySource.includes('url.searchParams.set("mode", "manifest")'), "manifest endpoint does not use the lightweight Apps Script mode");
...
console.log("Validated v103: reports publish by button, preload once, persist in browser, and refresh only when snapshot_version changes");
