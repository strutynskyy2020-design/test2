...
const REPORT_CACHE_API_VERSION = "v121-deposit-projection-block-detection";
...
function scoreDepositProjectionBlock(values, headerRowIndex, teamColumnIndex, loginColumnIndex, valueColumnIndex) {
  let currentTeamKey = "";
  let teamCount = 0;
  let summaryCount = 0;
  let operatorCount = 0;
  ...
  for (let rowIndex = headerRowIndex + 1; rowIndex < lastRowIndex; rowIndex += 1) {
    ...
    if (currentTeamKey && isDepositProjectionLogin(rawLogin) && isDepositProjectionValue(rawValue)) {
      operatorCount += 1;
      rowsWithData += 1;
    }
  }
...
function findDepositProjectionBlock(values) {
  ...
  for (let rowIndex = 0; rowIndex < values.length; rowIndex += 1) {
    ...
    for (let columnIndex = 0; columnIndex < row.length; columnIndex += 1) {
      ...
      // The current production layout is Team | Login | Result, but try a few
      // nearby offsets so inserted helper columns do not silently break reports.
      for (let loginOffset = 1; loginOffset <= 3; loginOffset += 1) {
        for (let valueOffset = loginOffset + 1; valueOffset <= loginOffset + 3; valueOffset += 1) {
          const loginColumnIndex = columnIndex + loginOffset;
          const valueColumnIndex = columnIndex + valueOffset;
          ...
          candidates.push({
            ...
            login_column_index: loginColumnIndex,
...
function getDepositProjectionLeaderboard(spreadsheet) {
  ...
  for (let rowIndex = block.header_row_index + 1; rowIndex < lastRowIndex; rowIndex += 1) {
    ...
    if (!currentTeamKey || !isDepositProjectionLogin(rawLogin) || !isDepositProjectionValue(rawValue)) continue;
    rows.push({
  ...
  const reports = logins.map((goalsLogin) => {
    ...
    return {
      ...
      payload: {
        ...
        deposit_issuances: getDepositIssuanceRows(goalsLogin, depositGiving),
        schedule,
      },
    };
  });
